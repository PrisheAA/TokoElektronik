...
class M_Transaksi extends CI_Model {
	...
	public function cekstok(){
		...
		for($i=0;$i<count($this->cart->contents());$i++){
			...
			$stok = $this->db->where('kode_elektronik', $this->input->post('kode_elektronik')[$i])->get('elektronik')->row()->stok;
	...
	public function simpan_db(){
			...
			$this->db->where('kode_elektronik', $this->input->post('kode_elektronik')[$i])->update('elektronik', $u_stok);
			}
			...
			$s_dttransaski[] = array('kode_transaksi' => $ts->kode_transaksi,
									 'kode_elektronik'=>$this->input->post('kode_elektronik')[$i],
		...
		public function ambil_dts($kode_transaksi){
			...
			return $this->db->join('elektronik', 'elektronik.kode_elektronik = detail_transaksi.kode_elektronik')
