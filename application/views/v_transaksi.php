	...
	<div class="col-md-7">
		...
		<table class="table table-hover table-stripped"> 
			...
			<thead>
				...
				<td>no</td><td>merk_elektronik</td><td>tahun</td><td>kategori</td><td>harga</td><td>cover</td><td>buatan</td><td>produksi</td><td>stok</td><td></td><td></td>
			...
			<tbody>
				...
				<tr>
					...
					<td><?=$no?></td></td><td><?=$lk->merk_elektronik?></td><td><?=$lk->tahun?></td><td><?=$lk->nama_kategori?></td><td><?=$lk->harga?></td><td><img src="<?=base_url('assets/gambar/'.$lk->cover)?>" style="width:40px;"></td><td><?=$lk->buatan?></td><td><?=$lk->produksi?></td><td><?=$lk->stok?></td>

					<td> <a href="<?=base_url('index.php/Cart/tambah_cart/'.$lk->kode_elektronik)?>" class="btn btn-success">Beli</a></td>
...
<table class="table table-stripped table-hover"> 
	...
	<tr><td colspan="2">Nama Pembeli</td><td colspan="4"><input type="text" name="pembeli" value="<?=$this->session->flashdata('pembeli');?>"></td></tr>
				...
				<th>No</th><th>merk_elektronik</th><th>Jumlah</th><th>Harga</th><th>Subtotal</th><th></th>
			...
			<?php $no = 0; foreach($this->cart->contents() as $ct): $no++;?>
				...
				<input type="hidden" name="kode_elektronik[]" value="<?=$ct['id']?>">
